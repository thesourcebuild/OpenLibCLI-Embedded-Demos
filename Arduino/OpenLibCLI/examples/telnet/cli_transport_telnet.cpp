...
static Client *telnet_client_from_ctx(cli_telnet_ctx_struct_t *ctx);
...
void cli_transport_telnet_init(cli_telnet_ctx_struct_t *ctx, cli_transport_struct_t *transport, void *client_handle) {
  if (!ctx || !transport) {
    return;
  }
...
int8_t cli_telnet_handler(cli_telnet_ctx_struct_t *ctx) {
  ...
  if (!ctx || !client) {
    ...
    while (client->available() > 0 && ctx->rx_len < sizeof(ctx->rx_buf)) {
      ...
      if (raw >= 0) {
        uint8_t out;
        if (cli_telnet_process_byte(ctx, (uint8_t)raw, &out)) {
