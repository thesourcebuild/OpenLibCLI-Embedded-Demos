...
#define CLI_LED_DEINIT() cyw43_arch_deinit()
...
#define CLI_DEMO_BANNER_SERIAL                                       \
  "************************************************************\r\n" \
  "*          OpenLibCLI  --  Serial (UART) on Pico           *\r\n" \
...
void cli_demo_setup(void)
{
  ...
  cli_init(&s_cli, "Pico", &transport, &platform, s_cmd_pool, sizeof(s_cmd_pool) / sizeof(s_cmd_pool[0]));
...
static int8_t cmd_show_version(cli_struct_t *cli, const char *cmd, cli_argc_t argc, const char *argv[])
{
  ...
  cli_println(cli, "Platform   : Raspberry Pi Pico");
  cli_println(cli, "Transports : Serial (UART via stdio)");
  return CLI_OK;
}
