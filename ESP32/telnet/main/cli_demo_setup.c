...
#define CLI_LED_INIT()                                  \
  do                                                    \
  {                                                     \
    gpio_reset_pin(CLI_LED_GPIO);                       \
    gpio_set_direction(CLI_LED_GPIO, GPIO_MODE_OUTPUT); \
    gpio_set_level(CLI_LED_GPIO, 0);                    \
  } while (0)
...
#define CLI_DEMO_BANNER_TELNET                                       \
  "************************************************************\r\n" \
  "*         OpenLibCLI  --  Telnet (WiFi) on ESP32          *\r\n"  \
...
void cli_demo_setup(int client_fd)
{
  ...
  cli_transport_telnet_init(&s_telnet_ctx, &s_transport, client_fd);
  cli_transport_telnet_negotiate(&s_telnet_ctx);

  cli_init(&s_cli, "ESP32", &s_transport, &platform,
...
void cli_demo_shutdown(void)
{
  ...
  {
    cli_done(&s_cli);
    if (s_telnet_ctx.client_fd >= 0)
    {
      close(s_telnet_ctx.client_fd);
      s_telnet_ctx.client_fd = -1;
    }
    s_initialised = false;
  }
}
                               ...
                               cli_argc_t argc, const char *argv[])
{
  ...
  cli_println(cli, "Platform   : ESP32 (ESP-IDF, Telnet over WiFi)");
  cli_println(cli, "Transports : Telnet (TCP :2323 via BSD sockets)");
  return CLI_OK;
}
